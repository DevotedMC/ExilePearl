...
class PlayerSuicideTask implements Runnable, SuicideHandler {
	 ...
	 */
	public PlayerSuicideTask(final ExilePearlPlugin plugin) {
	...
	public void start() {
		if (enabled) {
			plugin.log(Level.WARNING, "Tried to start the player suicide task but it was already started.");
			return;
		}
		
		taskId = plugin.getServer().getScheduler().scheduleSyncRepeatingTask(plugin, this, TICKS_PER_SECOND, TICKS_PER_SECOND);
		if (taskId == -1) {
			plugin.log(Level.SEVERE, "Failed to start the player suicide task");
			return;
		} else {
			players.clear();
			enabled = true;
			plugin.log("Started the player suicide task");
		}
	}
	...
	public void stop() {
		if (enabled) {
			...
			enabled = false;
			taskId = 0;
			plugin.log("Stopped the player suicide task");
		}
	}
	...
	public void addPlayer(PearlPlayer player) {
		int timeout = plugin.getPearlConfig().getSuicideTimeoutSeconds();
